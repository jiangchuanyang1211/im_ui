    ...
    >
      ...
      <el-table-column prop="icon" label="图标" align="center" width="100">
        <template slot-scope="scope">
          <svg-icon :icon-class="scope.row.icon ||'component'" />
        </template>
      </el-table-column>
        ...
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item prop="menuCode">
              <el-input v-model="form.menuCode" placeholder="请输入菜单编码" maxlength="255" />
              <span slot="label">
                <el-tooltip content='菜单唯一编码，用于记录后端日志' placement="top">
...
export default {
  ...
  data() {
    ...
    let validateMenuCode = function(rule, value, callback){
      if(!value || value==""){
        callback(new Error('请输入菜单编码'));
      }else{
        const regex = /^[A-Z_]+$/;
        if(!regex.test(value)){
          callback(new Error('请输入菜单编码包含数字或字母'));
        }else{
          callback()
        }
    ...
    return {
      ...
      // 查询参数配置列表
      paramList: [
        {name: "menuName", label: "菜单名称", type: "input", param: "", placeholder: "请输入菜单名称"},
        {name: "status", label: "菜单状态", type: "select", param: "", placeholder: "菜单状态", dict: "sys_normal_disable"}
      ],
      // 菜单表格树数据
      menuList: [],
      // 菜单树选项
      ...
      rules: {
        ...
        menuCode: [
          { validator: validateMenuCode,required: true,trigger: "blur" }
        ]
  ...
  methods: {
    ...
    handleAdd(row) {
      ...
      this.open = true;
      this.title = this.$enum.ADD_MENU_TITLE;
    },
    /** 展开/折叠操作 */
    toggleExpandAll() {
    ...
    handleUpdate(row) {
      ...
      getMenu(row.id).then(response => {
        ...
        this.open = true;
        this.title = this.$enum.EDIT_MENU_TITLE;
      });
    },
    /** 提交按钮 */
    submitForm: function() {
