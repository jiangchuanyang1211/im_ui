...
export default {
  ...
  data() {
    return {
      ...
      column: [
        { name: 'id', label: '字典编码',visible: true  },
        { name: 'dictLabel', label: '字典标签',visible: true  },
        { name: 'dictValue', label: '字典键值',visible: true  },
        { name: 'dictSort', label: '字典排序',visible: true  },
        { name: 'status', label: '状态',visible: true , slot:true },
  ...
  created() {
    this.queryParams.dictType = this.$route.params && this.$route.params.dictype
    this.defaultDictType = this.queryParams.dictType
    this.getTypeList();
  },
  methods: {
    /** 查询字典类型详细 */
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = this.$enum.ADD_DICT_TITLE;
    ...
    handleUpdate(row) {
      ...
      getData(id).then(response => {
        ...
        this.open = true;
        this.title = this.$enum.EDIT_DICT_TITLE;
      });
    },
    /** 提交按钮 */
    submitForm: function() {
    ...
    handleDelete(row) {
      const id = row.id || this.ids;
      this.$modal.confirm('是否确认删除字典编码为"' + id + '"的数据项？').then(function() {
        return delData(id);
      }).then(() => {
        this.getList();
