  ...
  <div class="login">
    <el-form ref="loginForm" :model="loginForm" :rules="loginRules" class="login-form">
      ...
      <el-form-item prop="smsCode" v-if="captchaEnabled">
        <phone-code :loginForm="loginForm"></phone-code>
...
export default {
  name: "Login",
  components:{PhoneCode,SliderVertify},
  data() {
    ...
    getCurrentRoute(routes){
       if(!routes || routes.length<1){
          ...
          if(routes[0].children){
            return `${routes[0].path}/${routes[0].children[0].path}`
          }
        ...
        if (valid) {
          ...
          this.$store.dispatch("Login", this.loginForm).then(res => {
            if(res.data.changePwd){
              ...
              getRouters().then(res => {
                let currentRoute = this.getCurrentRoute(res?.data);
                this.$store.commit('SET_ISLOGIN',true);
                localStorage.setItem('isLogin','isLogin')
                this.$router.push({ path: currentRoute || this.redirect || "/struct/address" }).catch(()=>{});
             })
            }
          }).catch((res) => {
            this.loading = false;
