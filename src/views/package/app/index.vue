  ...
  <div class="app-container">
    ...
    >
      ...
      <template v-slot:status="{ RowInfo }">
        <dict-tag 
          v-if="!checkPermi(['package:app:enable'])"
    ...
    <!-- APP包新增弹窗 -->
    <app-package-add ref="appPackageAdd" @operSuccess="getList">
    </app-package-add>
    <!-- APP包详情弹窗 -->
    <app-package-detail ref="appPackageDetail" >
    </app-package-detail>
...
import { listApp, delApp, enableApp, disableApp, detailApp } from "@/api/package/app";
import AppPackageAdd from "./dialog/add.vue";
import AppPackageDetail from "./dialog/detail.vue";

export default {
  name: "appPackage",
  dicts: ["im_app_package", "sys_yes_no", "package_normal_disable"],
  components: {
    AppPackageAdd,
    AppPackageDetail
  },
  data() {
    return {
      ...
      // 列信息
      columns: [
        { name: "appName", key: 0, label: `应用名称`, visible: true },
        { name: "packageVersion", key: 2, label: `版本号`, visible: true },
        { name: "packageName", key: 3, label: `包名称`, visible: true },
        { name: "size", key: 4, label: `大小`, visible: true, slot:true },
        { name: "force", key: 5, label: `强制更新`, visible: true, slot: true },
  ...
  methods: {
    ...
    handleAdd() {
      ...
      this.$refs.appPackageAdd.open("新增APP包", _form)
    },
    /** 详情按钮操作 */
    handleDetail(row) {
      this.$refs.appPackageDetail.open("APP包详情", row.id)
    },
    // 启用/停用APP包
    handleStatus(row) {
      ...
      this.$modal.confirm(
        ...
        return row.status == true ? enableApp(row.id) : disableApp(row.id)
      }).then(() => {
