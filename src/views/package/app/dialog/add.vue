      ...
      <el-row :gutter="20">
        <el-col :span="12">
          <el-form-item label="应用名称" prop="appName">
            <el-input v-model.trim="form.appName" controls-position="right" :min="0" placeholder="请输入应用名称"/>
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item label="版本号" prop="packageVersion">
            <el-input v-model.trim="form.packageVersion" placeholder="请输版本号"/>
          </el-form-item>
        </el-col>
      </el-row>
...
import { addApp, checkAppVersion } from "@/api/package/app";
  ...
  data() {
    ...
    const checkVersion = async(rule, value, callback)=> {
      if(value) {
        ...
        await checkAppVersion(params).then( res => {
          if(res.data == 0) {
            callback()
          } 
          else {
