<template>
  <div class="app-container">
    ...
    <ZfTable ref="myTable" :table-head-titles="columns" :isNeedSelection="true" :isSearch="isSearch" :url="url"
      :query-params="queryParams" @handleSelectionChange="handleSelectionChange">
      <template v-slot:jobGroup="{ RowInfo }">
        <dict-tag :options="dict.type.sys_job_group" :value="RowInfo.row.jobGroup" />
      </template>
      <template v-slot:status="{ RowInfo }">
        <el-switch v-model="RowInfo.row.status" active-value="0" inactive-value="1"
      ...
      <template v-slot:default="{ RowInfo }">
        ...
        <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(RowInfo.row)"
          v-hasPermi="['monitor:job:remove']">删除</el-button>
        <el-dropdown size="mini" @command="(command) => handleCommand(command, RowInfo.row)"
              ...
              <span slot="label">
                ...
                <el-tooltip placement="top">
                  <div slot="content">
                    ...
                    <br />Class类调用示例：com.tsenf.quartz.task.RyTask.ryParams('ry')
...
export default {
  ...
  data() {
    return {
      ...
      // 查询参数配置列表
      paramList: [
        {name: "jobName", label: "任务名称", type: "input", param: "", placeholder: "请输入任务名称"},
        {name: "jobGroup", label: "任务组名", type: "input", param: "", placeholder: "请输入任务组名"},
        {name: "status", label: "状态", type: "select", param: "",  placeholder: "请选择任务状态", dict: "sys_job_status"},
      ],
      // 总条数
      total: 0,
      // 定时任务表格数据
      ...
      // 列信息
      columns: [
        { name: "jobId", key: 0, label: `任务编号`, visible: true, width: 100 },
        { name: "jobName", key: 1, label: `任务名称`, visible: true },
        { name: "configName", key: 2, label: `任务组名`, visible: true },
        { name: "invokeTarget", key: 3, label: `调用目标字符串`, visible: true },
        { name: "cronExpression", key: 4, label: `cron执行表达式`, visible: true, slot: true },
        { name: "status", key: 4, label: `状态`, visible: true },
      ],
      // 查询方法
      url: (parms) => {
        return listJob(parms);
      },
      // 查询调用标识
      isSearch: false,
      // 表单参数
