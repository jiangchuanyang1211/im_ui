<template>
  <div class="app-container">
    ...
    <!-- 搜索行 -->
    <base-search :paramList="paramList" @handleQuery="handleQuery">
    </base-search>
  ...
  data() {
    return {
      ...
      tableHeadTitles: [
          ...
          { name: 'tokenId', label: '会话编号', visible: true },
          { name: 'userName', label: '登录名称',  visible: true},
          { name: 'ipaddr', label: '主机' , visible: true},
          { name: 'loginTime', label: '登录时间', visible: true },
      ],
      url: (parms) => {
        return list(parms);
      },
