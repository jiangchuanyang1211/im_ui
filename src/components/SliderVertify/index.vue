...
import { getSliderImg } from "@/api/login";
export default {
  name: 'SliderVertify',
  props: {
      // 阻塞块长度
  ...
  methods: {
      ...
      getCaptcha() {
          ...
          //取后端默认值
          const data = {};
          getSliderImg(data).then((response) => {
      ...
      createImage(onload) {
          ...
          image.onload = onload;
          image.onerror = () => {
              image.src = require('../../assets/404_images/404_cloud.png');
          };
      ...
      endEvent(originX) {
          ...
          if (this.isFrontCheck) {
              ...
              if (!spliced) {
                  ...
                  // 设置特殊值，后台特殊处理，直接验证通过
                  this.$emit('success', {uuid: this.uuid, code: moveLength});
              }
          } else {
              this.$emit('success', {uuid: this.uuid, code: moveLength});
          }
